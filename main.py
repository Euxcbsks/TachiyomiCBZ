...
parser.add_argument(
    "-d",
    "--delete",
    help="flag to delete the original chapter folder + images once the .cbz file has been created. Default = no deletions",
    action="store_true",
)
parser.add_argument(
    "-t",
    "--test",
    help="flag to only print the paths of all the .cbz files to be created but do not create them. Default = create the CBZ",
    action="store_true",
)
parser.add_argument(
    "-c",
    "--clean",
    help="flag to clean up by deleting any .nomedia files in chapter folders that it comes across. Default = no cleanup",
    action="store_true",
)
parser.add_argument(
    "-m",
    "--merge",
    help="flag to merge all chapters into a single .cbz file. Default = no merge",
    action="store_true",
)
parser.add_argument(
    "-v",
    "--verbosity",
    help="level of verbosity [0|1|2|3]. Default = 0 (silent)",
    default=0,
)
...
NUMBER_FILES_MERGED = (
    0  # to avoid filename conflicts, the number of files to merge must be kept track of
)
...
def cbz_folder(folder_name: str):
    ...
    if not isMerge:
        ...
        if not isTest:
            zip_name = shutil.make_archive(
                folder_name, "zip", os.path.join(os.getcwd(), folder_name)
            )
            p = Path(zip_name)
        ...
        if not isTest:
            ...
            for fname in files:
                file_extension = Path(
                    os.path.join(os.getcwd(), folder_name, fname)
                ).suffix
                shutil.copy(
                    os.path.join(os.path.join(os.getcwd(), folder_name, fname)),
                    os.path.join(
                        ABSOLUTE_PATH,
                        "working_merge",
                        (str(NUMBER_FILES_MERGED) + file_extension),
                    ),
                )
    ...
    if isDelete:
        ...
        num_files_in_chapter_folder = len(
            [
                entry
                for entry in os.listdir(current_chapter_full_path)
                if os.path.isfile(os.path.join(current_chapter_full_path, entry))
            ]
        )
        VERBOSITY_PRINT(
            2,
            f"Deleting {current_chapter_full_path} ({num_files_in_chapter_folder} files)",
        )
    ...
    for folder_name in folder_list:
        ...
        if directory_only_list:  # the folder being examined has directories in it - it is not a chapter so explore the directories
            ...
            if can_cbz_folder:
                ...
                VERBOSITY_PRINT(
                    3,
                    f"Skipping {os.path.join(os.getcwd(), folder_name)} - archive files are already present in this folder",
                )
                NUMBER_SKIPPED += 1
...
def main():
    ...
    ABSOLUTE_PATH = os.path.abspath(
        tachiyomi_path
    )  # ABSOLUTE_PATH is used for merge operations to maintain a reference to the root of the tachiyomi directory
    VERBOSITY_PRINT(
        3,
        f"tachiyomi_path = {tachiyomi_path}, isDelete = {isDelete}, isTest = {isTest}",
    )
    ...
    directory_list = [
        directory
        for directory in directory_list
        if os.path.isdir(os.path.join(tachiyomi_path, directory))
    ]
    ...
    if isMerge:  # merges all the files in the working directory and deletes the working directory
        zip_name = shutil.make_archive(
            os.path.join(ABSOLUTE_PATH, "merged"),
            "zip",
            os.path.join(ABSOLUTE_PATH, "working_merge"),
        )
        p = Path(zip_name)
        p.rename(p.with_suffix(".cbz"))
        NUMBER_ZIPPED += 1
    ...
    VERBOSITY_PRINT(
        0, f"   Number of .cbz files created:                          {NUMBER_ZIPPED}"
    )
    VERBOSITY_PRINT(
        0, f"   Number of folders skipped (archive already exists):    {NUMBER_SKIPPED}"
    )
    VERBOSITY_PRINT(
        0,
        f"   Number of files deleted:                               {NUMBER_FILES_DELETED}",
    )
    VERBOSITY_PRINT(
        0,
        f"   Elapsed time: {days}d{hours}h{minutes}m{remainder_secs + (elapsed_time.microseconds * (10**-6)):.3f}s",
    )
    print()
    ...
    if isCleanup:
        VERBOSITY_PRINT(
            0,
            "    .nomedia files WERE deleted in chapter folders before creating .cbz file",
        )
    else:
        VERBOSITY_PRINT(
            0,
            "    .nomedia files WERE NOT deleted in chapter folders before creating .cbz file",
        )
    if isDelete:
